#!/usr/bin/env python3
"""
build_materials.py  (v2 — restructured site)

Generates the teaching-materials pages for
https://dpdewey.github.io/LingMaterials/

Emits:
    TeachingMaterials/index.html                             (hub)
    TeachingMaterials/IntermediateEnglish/index.html          (catalogue)
    TeachingMaterials/AdvancedEnglish_for_Teachers/index.html
    TeachingMaterials/TeachingLessons/index.html
    TeachingMaterials/FamilyLessonPossessivePronouns/index.html

Navigation is SCOPED: these pages link to the site root and to each other,
never to the mentoring archive. A cooperating teacher never sees
departmental record-keeping links.

To add a material: add a line to the right `files` list below, then run
...
ROOT = os.environ.get("LINGMATERIALS_ROOT", ".")
BASE = "TeachingMaterials"
...
def navbar(page_title, subtitle, root, active_hub):
    """root = relative path to repo root ('../' or '../../')."""
    hub = ' class="active"' if active_hub else ""
  ...
  <div class="byu-navbar-inner">
    ...
    <nav>
      ...
      <a href="{root}{BASE}/index.html"{hub}>Teaching Materials</a>
      <a href="https://ling.byu.edu/">Department</a>
    </nav>
  </div>
</header>"""
...
def footers(root):
    """Teaching-scoped footer. No mentoring links."""
  ...
  <div class="area-footer-inner">
    ...
    <div>
      <h4>Collections</h4>
      <ul>
        <li><a href="{root}{BASE}/index.html">All Materials</a></li>
        <li><a href="{root}{BASE}/IntermediateEnglish/">Intermediate English</a></li>
        <li><a href="{root}{BASE}/AdvancedEnglish_for_Teachers/">Advanced English for Teachers</a></li>
        <li><a href="{root}{BASE}/TeachingLessons/">Classroom Lessons</a></li>
        <li><a href="{root}{BASE}/FamilyLessonPossessivePronouns/">Possessive Pronouns &amp; Family</a></li>
      </ul>
    </div>
  </div>
</footer>
...
def page(title, description, root, body):
...
<link rel="stylesheet" href="{root}styles-materials.css">
</head>
<body>
...
# (lesson plan / handout / survey / ...). These materials were authored as
# matched triads, so that grouping is real content structure, not decoration.
...
PROJECTS = [
    ...
    },
    {
        ...
        "note": (
            "<p>The handout and survey are large files (1.7&nbsp;MB and 3.3&nbsp;MB) because "
            "their images are embedded directly in the HTML. They may take a moment to load "
            "on a slow connection, but once loaded they work fully offline.</p>"
        ),
        "sets": [
            {
                "id": "possessives",
...
def build_project(project):
    ...
    body = f"""{navbar(project['name'], 'Teaching Materials', root, False)}

<section class="hero">
  <div class="hero-inner">
    <div class="mat-crumb"><a href="../index.html">&larr; All teaching materials</a></div>
...
{sets}
    ...
    out = os.path.join(ROOT, BASE, project["folder"], "index.html")
...
def build_hub():
    ...
    for p in PROJECTS:
        ...
        <div class="mat-project-foot">
          <span class="mat-project-count">{count_files(p)} files</span>
          <a href="{p['folder']}/" class="btn btn-primary">Open</a>
        </div>
      </article>""")
    total = sum(count_files(p) for p in PROJECTS)
    body = f"""{navbar('Teaching Materials', 'Department of Linguistics', root, True)}
  ...
  <section class="section">
    ...
    <div class="panels">
      <div class="panel">
        ...
        <p>
          These materials use their own visual design, distinct from the
          department pages that frame them. That is deliberate: they were built
          to hold a young learner's attention on a projector.
        </p>
      </div>
      <div class="panel">
    ...
    out = os.path.join(ROOT, BASE, "index.html")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w", encoding="utf-8") as fh:
        fh.write(page(
...
def build_all():
    return [build_hub()] + [build_project(p) for p in PROJECTS]
